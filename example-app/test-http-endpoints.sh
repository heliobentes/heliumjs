...
curl -X POST http://localhost:5173/webhooks/stripe \
...
curl -X POST http://localhost:5173/webhooks/generic \
  -H "Content-Type: application/json" \
...
curl -X GET "http://localhost:5173/auth/callback/github?code=abc123&state=xyz" \
  -s | jq .
echo ""

# Test 4: Dynamic OAuth Callback (Google)
echo "4. Testing GET /auth/callback/google?code=def456&state=qrs"
curl -X GET "http://localhost:5173/auth/callback/google?code=def456&state=qrs" \
  -s | jq .
echo ""
